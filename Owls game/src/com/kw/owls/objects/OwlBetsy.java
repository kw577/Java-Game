...
// Specjalna umiejetnosc - zbiera kwiatki za gracza - nalezy wyslac Betsy po kwiatka (przycik myszy)

public class OwlBetsy extends GameObject{
    ...
    // ask owl for help
    private boolean playerAsist = false; // okresla czy Betsy pomaga graczowi
    ...
    public OwlBetsy(int x, int y, ObjectId id, Handler handler, Game game) {
        super(x, y, id);
        ...
        try {
            owl_image = loader.loadImage("/owls/betsy_spritesheet.png");
    ...
    public void tick() {
    	...
    	if(this.playerAsist) {
            ...
            // granice planszy - musza byc ustawione gdyz jesli Betsy zblizy sie do granicy planszy
    ...
    private void wait_for_player() {
		// sprawdza czy player wybral Betsy do pomocy 
		...
		if(!game.isAskingForHelp() && !game.isDaisyHelping() && !game.isBetsyHelping() && !game.isZiggyHelping()) {
			...
			if(this.getBounds3().contains(new Point(game.getPlayer_click_x(),game.getPlayer_click_y()))) {
				...
				game.setOwlName("Betsy");
				this.askedForHelp = true;
				
				//System.out.println("\n\n\nBetsy will be helping you");
				//game.setBetsyHelping(true);
    ...
    private void set_velocity_vector(){
       ...
       //System.out.println("Owl Betsy - velX: " + velX + "  velY: " + velY + " dist: " + dist);
        ...
        for(int i = 0; i < handler.object.size(); i++) {
            ...
            if(tempObject.getId() == ObjectId.Block || tempObject.getId() == ObjectId.Ground) {
                ...
                if(getBounds().intersects(tempObject.getBounds())) {
                    ...
                    // zachowanie OwlBetsy po kolizji
                    this.x -= 3*velX;
                    this.y -= 3*velY;
            ...
            if(game.getPlayer_click_x() > 0 && game.getPlayer_click_y() > 0 && tempObject.getId() == ObjectId.UsualFlower) {
            	if(getBounds4().intersects(tempObject.getBounds())) {
            		
	            		game.setFlowers(game.getFlowers() + 1); // + 1pkt do punktacji gry
            ...
            if(game.getPlayer_click_x() > 0 && game.getPlayer_click_y() > 0 && tempObject.getId() == ObjectId.WaterFlower) {
            	if(getBounds4().intersects(tempObject.getBounds())) {
            ...
            if(game.getPlayer_click_x() > 0 && game.getPlayer_click_y() > 0 && tempObject.getId() == ObjectId.RareFlower) {
            	if(getBounds4().intersects(tempObject.getBounds())) {
    ...
    public void render(Graphics g) {
        ...
        // zaznacza punkt wybrany przez gracza do sterowania ruchem OwlBetsy
       
        if(this.playerAsist && game.getPlayer_click_x() > 0 && game.getPlayer_click_y() > 0) {
        ...
        ///// renderowanie Betsy gdy pomaga graczowi
        if(this.playerAsist) {
        ...
        // renderowanie gdy Betsy gdy czeka na gracza
        if(!this.playerAsist) {
        	g.setColor(Color.white);
        	if((handler.getPlayer_x() - this.x) >= 0) { // Betsy patrzy w prawo
        		g.fillOval((int) (x + 15), (int) (y+5), 17, 17);
	        	g.drawImage(owlSpriteSheet.grabImage((int)(1), 4, 100, 50), (int)(x - 37), (int)(y-7), null);
        	}
        	else { // Betsy patrzy w lewo
        		g.fillOval((int) (x), (int) (y+5), 17, 17);
   ...
   private void choose_direction(){
       // (this.x + width/2)/32 - 1) - skanuje teren dookola OwlBetsy - start w pkt (-1, -1) wzg polozenia OwlBetsy
       this.readMap((int)((this.x + width/2)/32 - 1), (int)((this.y + height/2)/32 - 1), 3, 3);
    
       //System.out.println("OwlBetsy - Tabela terenu otaczajacego");
       ...
       */
       ...
       //System.out.println("OwlBetsy - Tabela terenu otaczajacego z priorytetem wyboru pol");
       for(int xx = 0; xx < 3; xx++) {
           for(int yy = 0; yy < 3; yy++) {
               //System.out.print(surrunding_area[yy][xx] + " ");
           }
       ...
       if (collision_dir1_counter > 5) {
           ...
           //System.out.println("/n####################### UWAGA #################################3");
            //System.out.println("/nOwlBetsy - Tabela terenu otaczajacego z uwzglednieniem sekwencyjnych kolizji");
               for(int xx = 0; xx < 3; xx++) {
                   for(int yy = 0; yy < 3; yy++) {
                       //System.out.print(surrunding_area[yy][xx] + " ");
                   }
    ...
    // uzywana do sprawdzenia czy player wybral Betsy do wspolpracy
    ...
    // do zbierania kwiatkow
    public Rectangle getBounds4() {
    		
    	if(turned_right)
    		return new Rectangle((int)(x - 5), (int)(y - 5), (int)(width + 25), (int)(height + 10));
    	
    	else
    		return new Rectangle((int)(x - 20), (int)(y - 5), (int)(width + 25), (int)(height + 10));
    }
    
    private void direction() {
		// pomocniczo do renderowania animacji - sprawdza czy Betsy porusza sie w lewo czy w prawo
		if(this.velX < 0) {
			turned_right = false;
			turned_left = true;
		}
