...
public class OwlZiggy extends GameObject{
    ...
    // ask owl for help
    private boolean playerAsist = false; // okresla czy Ziggy pomaga graczowi
    ...
    public OwlZiggy(int x, int y, ObjectId id, Handler handler, Game game) {
        ...
        try {
            owl_image = loader.loadImage("/owls/ziggy_spritesheet.png");
    ...
    public void tick() {
    	...
    	if(this.playerAsist) {
            ...
            // granice planszy - musza byc ustawione gdyz jesli Ziggy zblizy sie do granicy planszy
	...
	private void wait_for_player() {
		...
		if(!game.isAskingForHelp() && !game.isDaisyHelping() && !game.isBetsyHelping() && !game.isZiggyHelping()) {
			...
			if(this.getBounds3().contains(new Point(game.getPlayer_click_x(),game.getPlayer_click_y()))) {
				...
				game.setOwlName("Ziggy");
				this.askedForHelp = true;
				
				//System.out.println("\n\n\nZiggy will be helping you");
				//game.setZiggyHelping(true);
    ...
    private void set_velocity_vector(){
        ...
        //System.out.println("Owl Ziggy - velX: " + velX + "  velY: " + velY + " dist: " + dist);
        ...
        for(int i = 0; i < handler.object.size(); i++) {
            ...
            if(tempObject.getId() == ObjectId.Block || tempObject.getId() == ObjectId.Ground) {
                ...
                if(getBounds().intersects(tempObject.getBounds())) {
                    ...
                    // zachowanie OwlZiggy po kolizji
                    this.x -= 3*velX;
                    this.y -= 3*velY;
    ...
    public void render(Graphics g) {
        ...
        // zaznacza punkt wybrany przez gracza do sterowania ruchem OwlZiggy
        ...
        ///// renderowanie Ziggy gdy pomaga graczowi
        if(this.playerAsist) {
        ...
        // renderwanie Ziggy gdy czeka na gracza
        if(!this.playerAsist) {
        	g.setColor(Color.white);
        	if((handler.getPlayer_x() - this.x) >= 0) { // Ziggy patrzy w prawo
        		g.fillOval((int) (x + 15), (int) (y+5), 17, 17);
	        	g.drawImage(owlSpriteSheet.grabImage((int)(1), 4, 100, 50), (int)(x - 37), (int)(y-7), null);
        	}
        	else { // Ziggy patrzy w lewo
        		g.fillOval((int) (x), (int) (y+5), 17, 17);
   ...
   private void choose_direction(){
       // (this.x + width/2)/32 - 1) - skanuje teren dookola OwlZiggy - start w pkt (-1, -1) wzg polozenia OwlZiggy
       this.readMap((int)((this.x + width/2)/32 - 1), (int)((this.y + height/2)/32 - 1), 3, 3);
    
       //System.out.println("OwlZiggy - Tabela terenu otaczajacego");
       ...
       */
       ...
       //System.out.println("OwlZiggy - Tabela terenu otaczajacego z priorytetem wyboru pol");
       for(int xx = 0; xx < 3; xx++) {
           for(int yy = 0; yy < 3; yy++) {
               //System.out.print(surrunding_area[yy][xx] + " ");
           }
       ...
       if (collision_dir1_counter > 5) {
           ...
           //System.out.println("/n####################### UWAGA #################################3");
            //System.out.println("/nOwlZiggy - Tabela terenu otaczajacego z uwzglednieniem sekwencyjnych kolizji");
               for(int xx = 0; xx < 3; xx++) {
                   for(int yy = 0; yy < 3; yy++) {
                       //System.out.print(surrunding_area[yy][xx] + " ");
                   }
    ...
    // uzywana do sprawdzenia czy player wybral Ziggy do wspolpracy
    ...
    private void direction() {
		// pomocniczo do renderowania animacji - sprawdza czy Ziggy porusza sie w lewo czy w prawo
		if(this.velX < 0) {
			turned_right = false;
			turned_left = true;
		}
